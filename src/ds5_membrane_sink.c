...
"<p class=sub>Rumble as Subwoofer &middot; analyser &amp; tweak &middot; <span id=stat>connecting&hellip;</span></p>"
"<canvas id=cv width=720 height=200></canvas>"
"<div class=legend><span class=sw style=background:#d98a2b></span>Haptic (bass &lt; cutoff)"
"<span class=sw style=background:#3a7ca5></span>Membrane (&ge; cutoff)"
"<span style=color:#c0392b;margin-left:12px>&#9474;</span> cutoff</div></div>"
"<div class=card>"
"<div style='font-size:13px;color:#555'>Membrane level</div><div class=meter><div id=mm style=background:#3a7ca5></div></div>"
"<div style='font-size:13px;color:#555'>Haptic level</div><div class=meter><div id=mh style=background:#d98a2b></div></div>"
"<div style='font-size:13px;color:#555'>Mic level</div><div class=meter><div id=mi style=background:#5a9e5a></div></div>"
"<div class=row><label>Output</label><input type=checkbox id=jack><output id=jack_v></output></div>"
"<div style='font-size:11px;color:#999;margin-top:-4px'>Headphone jack (on) instead of the internal membrane (off). Jack = true stereo.</div>"
...
"<div class=row><label>Microphone</label><input type=checkbox id=mic><output id=mic_v></output></div>"
"<div style='font-size:11px;color:#999;margin-top:-4px'>Voice-in recording device. Note: disturbs the gamepad input.</div>"
"<div class=row><label>LED analyser</label><input type=checkbox id=leds><output id=leds_v></output></div>"
"<div style='font-size:11px;color:#999;margin-top:-4px'>Lightbar colour = spectrum (bass=blue, mid=red, high=white), 5 LEDs = VU.</div>"
"</div><script>"
...
"document.getElementById('on_v').textContent=e.target.checked?'on':'off';send('on',e.target.checked?1:0);});"
"document.getElementById('mic').addEventListener('change',e=>{"
"document.getElementById('mic_v').textContent=e.target.checked?'on':'off';send('mic',e.target.checked?1:0);});"
"document.getElementById('jack').addEventListener('change',e=>{"
"document.getElementById('jack_v').textContent=e.target.checked?'Jack':'Membrane';send('jack',e.target.checked?1:0);});"
"document.getElementById('leds').addEventListener('change',e=>{"
"document.getElementById('leds_v').textContent=e.target.checked?'on':'off';send('leds',e.target.checked?1:0);});"
...
"document.getElementById('on_v').textContent=m.on?'on':'off';"
"let mc=document.getElementById('mic');mc.checked=!!m.mic;"
"document.getElementById('mic_v').textContent=m.mic?'on':'off';"
"let jk=document.getElementById('jack');jk.checked=!!m.jack;"
"document.getElementById('jack_v').textContent=m.jack?'Jack':'Membrane';"
"let ld=document.getElementById('leds');ld.checked=!!m.leds;"
"document.getElementById('leds_v').textContent=m.leds?'on':'off';"
...
"ws.onopen=()=>st('connected');ws.onclose=()=>{st('disconnected \\u2014 reconnecting\\u2026');init=false;setTimeout(conn,1000);};"
